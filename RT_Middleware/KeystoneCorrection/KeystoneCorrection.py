...
class KeystoneCorrection(OpenRTM_aist.DataFlowComponentBase):
    ...
    def onExecute(self, ec_id):
        if self._ImageInIn.isNew():  # 新しいデータが来たか確認
            ...
            self.Acceptanceimg[0] = cv2.cvtColor(
                self.Acceptanceimg[0], cv2.COLOR_BGRA2BGR)
            ...
            if self.counter == 4:
                ...
                pts1 = np.float32([self.point_synthesis_numpy[0], self.point_synthesis_numpy[1],
                                  self.point_synthesis_numpy[2], self.point_synthesis_numpy[3]])
                pts2 = np.float32(
                    [[0, 0], [width, 0], [0, height], [width, height]])
                matrix = cv2.getPerspectiveTransform(pts1, pts2)
                imgoutput = cv2.warpPerspective(
                    self.img, matrix, (width, height))
                #cv2.imshow('Output image', imgoutput)
                self.img_jugge = 1

                for x in range(0, 4):
                    cv2.circle(
                        self.img, (self.circles[x][0], self.circles[x][1]), 3, (0, 255, 0), cv2.FILLED)
