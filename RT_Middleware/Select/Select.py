...
class Select(OpenRTM_aist.DataFlowComponentBase):
    ...
    def __init__(self, manager):
        ...
        """
		 ...
		 - Unit: ratio
		"""
        self._StartPointOut = OpenRTM_aist.OutPort(
            "StartPoint", self._d_StartPoint)
        ...
        self.pos = numpy.array([[0, 0], [self._Width[0], 0], [0, self._Height[0]], [
    ...
    def onClick(self, event):
        ...
        if (self.shiftState[0]):
            ...
            if (self.clickCount[0] == 0):
                self.pos = self.cache
                # reset pos
                self.canvas[0].coords(
                    self.points[0], 0 - 5, 0 - 5, 0 + 5, 0 + 5)
                self.canvas[0].coords(self.points[1], self._Width[0] -
                                      5, 0 - 5, self._Width[0] + 5, 0 + 5)
                self.canvas[0].coords(self.points[2], 0 - 5,
                                      self._Height[0] - 5, 0 + 5, self._Height[0] + 5)
                self.canvas[0].coords(self.points[3], self._Width[0] - 5,
                                      self._Height[0] - 5, self._Width[0] + 5, self._Height[0] + 5)

    def onDrag(self, event):
        self.canvas[0].coords(self.rect[0], self.startX[0],
                              self.startY[0], event.x, event.y)
    ...
    ###
    ##
    # The shutdown action when E    xecutionContext stop
    ...
    def onActivated(self, ec_id):
        ...
        for i in range(0, 4):
            x = self.pos[i][0]
            y = self.pos[i][1]
            self.points[i] = self.canvas[0].create_oval(
                x - 5, y - 5, x + 5, y + 5, fill="blue")
    ...
    def onExecute(self, ec_id):
        ...
        if (self._ShowImageIn.isNew()):
            ...
            self.img[0] = numpy.frombuffer(rawImage.pixels, numpy.uint8).reshape(
                (rawImage.height, rawImage.width, 3))
            self.img[0] = cv2.cvtColor(self.img[0], cv2.COLOR_BGRA2BGR)

            # object identity
            # todo error when closing the window
            if (self.img[0] is not None):
                img = cv2.resize(
                    self.img[0], (self._Width[0], self._Height[0]))
                img = self.fit(
                    img, 1, self.pos[0], self.pos[1], self.pos[2], self.pos[3])
